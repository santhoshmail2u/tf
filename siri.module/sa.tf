resource "azurerm_storage_account" "sa_name1" {
  for_each                 = var.siri_sa
  name                     = each.value.name
  resource_group_name      = each.value.rg_n
...
resource "azurerm_storage_container" "cn_1" {
  storage_account_name = azurerm_storage_account.sa_name1["Sa1"].name
  count                = length(var.cn_name1)
  name                 = var.cn_name1[count.index]
}

resource "azurerm_storage_container" "cn_2" {
  storage_account_name = "${azurerm_storage_account.sa_name1["Sa2"].name}"
